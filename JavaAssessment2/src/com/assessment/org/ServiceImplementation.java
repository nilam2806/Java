...
public class ServiceImplementation implements IFeedbackService {
	...
	public List<Feedback> findFeedbacksByAuthorOrderById(String author) {
		return feedbackMap.values().stream().filter(feedback -> feedback.getDescription().startsWith(author))
				.sorted(Comparator.comparing(Feedback::getId)).collect(Collectors.toList());
	}
